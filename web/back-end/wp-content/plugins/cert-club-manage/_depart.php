...
require_once 'class-wpccm-history-table.php';

if (isset($_GET['clear_all_records'])) {
	global $wpdb;
	$db_table = DB_TABLE_WPCCM_HISTORY;
...
$data = array();
foreach ($raw as $d) {
	$d->is_match = $d->is_match == "y" ? __("Yes", "WPCCM") : "<span style='color:red;'>" . __("No", "WPCCM") . "<span>";
	$data[] = array('ID' => $d->id, 'openid' => $d->openid, 'keyword' => $d->keyword, 'is_match' => $d->is_match, 'time' => $d->time);
}

//Prepare Table of elements
$wp_list_table = new WPCCM_History_Table($data);
...
<div class="wrap">
	...
	<hr>
	<h2>
	 <?php _e('Statistics', 'WPCCM');?>
     <form action="" method="get" style="float:right;">
     <input type="hidden" name="page" value="wpccm-history-page" />
	 <button  id="clear_all_records" type="submit" name="clear_all_records" value="rows" class="add-new-h2"><?php _e("Clear All Records", "WPCCM");?></button>
	 </form>
	</h2>
    <br>
	<form action="" method="get">
		<input type="hidden" name="page" value="<?php echo WPCCM_HISTORY_PAGE; ?>" />
		<input type="hidden" name="per_page" value="<?php _e($per_page);?>" />
		<?php $wp_list_table->display();?>
	</form>
</div>
<script>document.getElementById("clear_all_records").onclick = function(){var r=confirm("<?php _e('Empty all the records ?', 'WPCCM');?>"); if(!r) return false;}</script>