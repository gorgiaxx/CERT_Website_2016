...
function prefix_ajax_add_foobar() {
	...
	$cateoptions = "<option value='default' value='default' class='select_cate_choose' >" . __('All categories', 'WPCCM') . "</option>";
	...
	//get posts
	$typeORcate = __('Category', 'WPCCM');
	$searchKeyInput = '';
	if (isset($_GET['key']) && trim($_GET['key']) != "") {
		$str = urldecode($_GET['key']);
		$searchKeyInput = trim($_GET['key']);
		$typeORcate = __('Type', 'WPCCM');
		$start = $offset;
	...
	switch ($_GET['rtype']) {
	case 'posts':
		$button_value = __('Insert', 'WPCCM') . '<span>' . __(' Content', 'WPCCM') . '</span>';
		break;
	case 'urls':
		$button_value = __('Insert', 'WPCCM') . '<span>' . __(' URL', 'WPCCM') . '</span>';
		break;
	case 'phmsg':
		$button_value = __('Sync', 'WPCCM');
		break;
	}
	$args = array(
		'public' => true,
	...
	_e('<input type="hidden" id="hidden_post_tid" value="' . $_GET['tid'] . '">
		...
		<div class="tablenav top">
         ...
         <input id="post-search-submit" class="button" type="submit" value="' . __('Search', 'WPCCM') . '"></input>
         </div>
	...
	if (count($posts_array) == 0) {
		_e("<thead><tr><th style='text-align:center;height: 77px;'>" . __('Search results is empty....', 'WPCCM') . "</th></tr></thead>");
	} else {
		_e("<thead><tr><th class='' width='40%'>" . __('Title', 'WPCCM') . "</th><th width='16%'><div sytle='text-align:center;'>" . $typeORcate . "</div></th><th width='22%'>" . __('Create Date', 'WPCCM') . "</th><th width='22%' style='text-align:center;'>" . __('Action', 'WPCCM') . "</th></tr></thead>
    	<tbody>");
		$i = 1;
...
function prefix_ajax_get_insert_content() {
	if ($_GET['rtype'] == 'posts') {
		...
		$myrow = get_post($_GET['postid']);
		$myrow->pic = WPCCM_PLUGIN_URL . '/img/' . $imageSize . '.png';
